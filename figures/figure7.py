...
fig_cont[1].set_title('[$B]_s$ (mM)')
fig_cont[2].set_title('[$Ca^{2+}]_s$ (mM)')
fig_cont[3].set_title('[$B]_s$ (mM)')
...
### Add horizontal and vertical dividing lines ###

# Adding one vertical line between the 2nd and 3rd columns
fig.add_artist(plt.Line2D([0.52, 0.52], [0.05, 0.925], color='black', linewidth=1))

# Adding three horizontal lines evenly spaced between rows
for y in [0.25, 0.49, 0.73]:  # Adjust for even spacing between rows
    fig.add_artist(plt.Line2D([0.08, 0.98], [y, y], color='black', linewidth=1))
...
plt.savefig('figures/figure7_prac.pdf')
plt.close()
